    ...
    @section('page')
        ...
        <header class="bg-main font-ubuntu relative">
            ...
            <div class="relative pt-[16rem] pb-[7rem] pl-5 space-y-5 text-slate-100">
                <h1 class="text-5xl font-bold uppercase md:text-7xl">{{ $hero->value['title'] ?? 'Tentang' }}</h1>
        ...
        <section class="bg-white lg:h-screen lg:flex lg:items-center font-ubuntu">
            <div
                ...
                <div class="relative w-full h-0" style="padding-top: 80%">
                    <img src="{{ asset('storage/' . ($hero->value['image'] ?? 'images/default.jpg')) }}" alt="Hero Image"
        ...
        <section class="relative bg-main bg-cover bg-center bg-fixed lg:h-auto font-ubuntu">
            ...
            <div class="relative z-10 flex items-center justify-center h-full lg:h-auto">
                <div class="text-center max-w-7xl px-4 lg:px-8 lg:py-16">
                    <div class="mt-8 mb-8 grid grid-cols-1 gap-14 lg:grid-cols-4 lg:gap-20">
                        @foreach ($counters->value as $counter)
                            <div class="space-y-2">
                                <h1 class="text-6xl text-slate-100 font-semibold">{{ $counter['value'] }}</h1>
                                <p class="text-slate-100 uppercase font-semibold whitespace-nowrap">
                                    {{ $counter['title'] }}
                                </p>
                            </div>
                        @endforeach
        ...
        <section class="bg-slate-200 font-ubuntu">
            <div class="py-20 px-6 space-y-6 lg:px-20">
                <div class="text-center space-y-4">
                    <h1 class="text-4xl font-bold text-gray-900 md:text-5xl">WE PROVIDE THE BEST OFFERS</h1>
                    ...
                    @foreach ($plans->value as $plan)
                        <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8">
                            ...
                            <div class="flex items-baseline text-gray-900">
                                ...
                                <span class="ms-1 text-xl font-normal text-gray-500">{{ $plan['duration'] }}</span>
                            </div>

                            <ul role="list" class="space-y-5 my-7">
                                @foreach ($plan['features'] ?? [] as $feature => $enabled)
                                    <li class="flex {{ $enabled ? '' : 'line-through decoration-gray-500' }}">
                                        <svg class="flex-shrink-0 w-4 h-4 {{ $enabled ? 'text-sky-800' : 'text-gray-400' }}"
                            ...
                            <button type="button"
                                class="text-white bg-sky-800 hover:bg-sky-900 focus:ring-4 focus:outline-none focus:ring-sky-200 font-medium rounded-lg text-sm px-5 py-2.5 inline-flex justify-center w-full text-center">
                                Choose Plan
                            </button>
                        </div>
                    @endforeach
